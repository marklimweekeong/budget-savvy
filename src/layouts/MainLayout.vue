<template>
	<q-layout view="hhh lpR fFf" :class="backgroundColorClass">
		...
		<!-- Body (TODO - Fix for desktop) -->
		<q-page-container v-if="deviceIsPhone" class="text-semidark page">
			<router-view />
		</q-page-container>
		<!-- Footer(Mobile only) -->
...
export default {
	components: {
		// Header,
		FooterLinks: () => import('../components/layout/FooterLinks.vue'),
		LinkList: () => import('../components/lists/LinkList.vue'),
		SettingsMenu: () => import('../components/layout/SettingsMenu.vue'),
	},
	data() {
		return {
			leftDrawerOpen: true,
